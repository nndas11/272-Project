...
import { useQuote } from "@/hooks/api/useMarket"
...
export function StockDialog({ symbol, onClose }: StockDialogProps) {
  ...
  const { data: quoteData } = useQuote(symbol, true)
  ...
  // Fetch initial quote using React Query hook
  useEffect(() => {
    if (quoteData) {
      setQuote({
        symbol: quoteData.symbol,
        last: quoteData.price ?? 0,
        ts: new Date(quoteData.ts || Date.now()).toISOString(),
      })
    }
  }, [quoteData])
